...
public class HaloDrive extends CommandBase {
    ...
    public HaloDrive(DriveTrain driveTrain) {
    ...
    public void drive() {
        ...
        SmartDashboard.putNumber("left trigger",
                RobotContainer.getInstance().getdriverController().getLeftTriggerAxis());

        // right arrow turns right, left arrow turns left

        if (RobotContainer.getInstance().getdriverController().getLeftTriggerAxis() > 0.2) {
            finesse = .5;
        }
        leftSpeed *= finesse;
        rightSpeed *= finesse;
    ...
    private double linearAcceleration(double speed, double input) {
        ...
        // if difference is positive AND less than the acceleration, just makes it equal
        // to the input
        if (difference > 0 && difference < acceleration) {
            return input;
        }

        // if difference is negative AND greater than decceleration (-acceleration),
        // just makes it equal to the input;
        if (difference < 0 && difference > -acceleration) {
            return input;
        }
    ...
    public void execute() {
        ...
        /*
         * SmartDashboard.putBoolean("self level activated",
         * RobotContainer.getInstance().getdriverController().getRightBumper());
